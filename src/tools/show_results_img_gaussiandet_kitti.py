...
base_dir = '/store/datasets/KITTIPoly/testing/image_2/'
...
for filename in os.listdir(results_dir):
    ...
    if os.path.isfile(complete_filename) and complete_filename.endswith(".txt"):
        with open(complete_filename, 'r') as f:
            lines = f.readlines()
        adress = lines[0].split()[0].split('_') #adress of mask
        filename = '_'.join(adress[:-1]) + '.png'
