...
# Cityscapes modules
# try:
from datasets.evaluation.IDDscripts.helpers.annotation import Annotation
from datasets.evaluation.IDDscripts.helpers.anue_labels import labels, name2label, id2label
# except:
...
# IDD files have a typical filename structure
# <city>/<frameNb>_<type>[_<type2>].<ext>
...
CsFile = namedtuple( 'csFile' , [ 'city'  , 'frameNb' , 'type' , 'type2' , 'ext' ] )
...
def getCsFileInfo(fileName):
    ...
    parts = [folder] + parts[:-1] + parts[-1].split('.')
    if not parts:
        printError( 'Cannot parse given filename ({}). Does not seem to be a valid IDD file.'.format(fileName) )
    if len(parts) == 5:
        csFile = CsFile( *parts )
    elif len(parts) == 4:
...
def getCoreImageFileName(filename):
    ...
    return "{}/{}".format( csFile.city , csFile.frameNb )
