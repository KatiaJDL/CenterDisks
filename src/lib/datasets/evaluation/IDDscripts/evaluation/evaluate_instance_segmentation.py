...
from datasets.evaluation.IDDscripts.helpers.anue_labels import *
import json

from datasets.evaluation.IDDscripts.evaluation.csHelpers import *
from datasets.evaluation.IDDscripts.evaluation.instances2dict import instances2dict
import numpy as np
import glob
...
#   - environment variable IDD_RESULTS
#   - environment variable IDD_DATASET/results
#   - ../../results/"
# (Remember to set the variables using "export IDD_<VARIABLE>=<path>".)
#
...
# <city>/123456_gtFine_instanceIds.png

def getPrediction( groundTruthFile , args ):
    #ground-truth file: whole path
    # determine the prediction path, if the method is first called
    if not args.predictionPath:
        rootPath = None
        if 'IDD_RESULTS' in os.environ:
            rootPath = os.environ['IDD_RESULTS']
        elif 'IDD_DATASET' in os.environ:
            rootPath = os.path.join( os.environ['IDD_DATASET'] , "results" )
        else:
    ...
    filePattern = "*{}/{}*.txt".format( csFile.city , csFile.frameNb )
    #print(filePattern)

    predictionFile = None
    for root, filenames in args.predictionWalk:
        #print(root)
        filenames_w_root = list(map(lambda file: os.path.join(root, file), filenames))
        #print(filenames_w_root)
        for filename in fnmatch.filter(filenames_w_root, filePattern):
...
# Where to look for IDD
if 'IDD_DATASET' in os.environ:
    args.IDDPath = os.environ['IDD_DATASET']
else:
    args.IDDPath = os.path.join(os.path.dirname(os.path.realpath(__file__)),'..','..')

# Where to save the results
if 'IDD_RESULTS' in os.environ:
    args.resultsPath = os.environ['IDD_RESULTS']
else:
    args.resultsPath = args.IDDPath

# Parameters that should be modified by user
args.exportFile         = os.path.join( args.resultsPath , "evaluationResults" , "resultInstanceLevelSemanticLabeling.json" )
args.groundTruthSearch  = os.path.join( args.IDDPath , "gtFine" , "val" , "*", "*_gtFine_instanceids.png" )
...
def evaluateImgLists(predictionList, groundTruthList, args):
    ...
    if args.JSONOutput:
        os.makedirs('../exp/IDD/polydet/from_cityscapes_TEST/results/evaluationResults', exist_ok=True)
...
def getAP():
    ...
    # evaluate
    res_dict = evaluateImgLists(predictionImgList, groundTruthImgList, args)
