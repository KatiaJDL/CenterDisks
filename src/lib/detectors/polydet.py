...
class PolydetDetector(BaseDetector):
  ...
  def process(self, images, return_time=False):
    with torch.no_grad():
      ...
      dets = polydet_decode(hm, polys, pseudo_depth, reg=reg, cat_spec_poly=self.opt.cat_spec_poly, K=self.opt.K, rep = self.opt.rep)
    if return_time:
  ...
  def show_results(self, debugger, image, results):
    debugger.add_img(image, img_id='polydet')
    for j in range(1, self.num_classes + 1):
      for bbox in results[j]:
        if bbox[4] > self.opt.vis_thresh:
          debugger.add_coco_bbox(bbox[:4], j - 1, bbox[4], img_id='polydet')
          debugger.add_polydet(bbox[5:-1], j - 1, bbox[4], img_id='polydet')
