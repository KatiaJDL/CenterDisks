...
class BaseDetector(object):
  ...
  def pre_process(self, image, scale, meta=None):
    ...
    if inp_image.shape[2] != 3:
      ...
      for i in range(int(inp_image.shape[2] / 3)):
        # print('inp_image.shape: ', inp_image.shape)
        inp_image[:, :, i*3:i*3+3] = ((inp_image[:, :, i*3:i*3+3] / 255. - self.mean) / self.std).astype(np.float32)
      # inp_image = inp_image.astype(np.float32)
      images = inp_image.transpose(2, 0, 1).reshape(1, inp_image.shape[2], inp_height, inp_width)
